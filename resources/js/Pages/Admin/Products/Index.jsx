...
const TYPE_LABEL = {
    ...
    service:        { label: 'Jasa/Layanan',  bg: 'bg-teal-100',   text: 'text-teal-700'   },
    rental_item:    { label: 'Item Rental',   bg: 'bg-cyan-100',   text: 'text-cyan-700'   },
    time_based:     { label: 'Berbasis Waktu',bg: 'bg-rose-100',   text: 'text-rose-700'   },
};
...
const FILTER_TYPES = {
    retail:      [['finished_goods','Barang Jadi'], ['raw_material','Bahan Baku'], ['combo','Combo/Paket']],
    fnb:         [['finished_goods','Menu/Makanan'], ['raw_material','Bahan Baku'], ['combo','Paket Menu']],
    service:     [['service','Layanan Jasa'], ['finished_goods','Produk Pendukung']],
    rental:      [['rental_item','Item Sewa'], ['finished_goods','Produk Pendukung']],
    ticket:      [['time_based','Tiket'], ['finished_goods','Produk Pendukung']],
    hospitality: [['time_based','Kamar/Unit'], ['service','Layanan'], ['finished_goods','Produk']],
    parking:     [['time_based','Tarif Parkir']],
    session:     [['time_based','Paket Sesi'], ['finished_goods','Produk Pendukung']],
};
...
function StockBadge({ product }) {
    ...
    return (
        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${low ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>
...
export default function Index({ products, storeType = 'retail' }) {
    ...
    // Kolom yang tampil per store type
    const showStock     = ['retail', 'fnb', 'rental'].includes(storeType);
    const showMargin    = ['retail', 'fnb'].includes(storeType);
    const showCapacity  = storeType === 'ticket';
    const showDuration  = ['session', 'parking'].includes(storeType);
    const showRateHour  = ['session', 'parking', 'rental', 'hospitality'].includes(storeType);
    const showDeposit   = storeType === 'rental';
    const showMaxGuests = storeType === 'hospitality';
    const showPrepTime  = storeType === 'fnb';
    ...
    const filtered = useMemo(() => {
        ...
        return products.filter((p) => {
            ...
            const matchStatus   = filterStatus === '' ? true : filterStatus === '1' ? p.is_active : !p.is_active;
    ...
    const stats = useMemo(() => ({
        ...
        lowStock: showStock
            ? products.filter((p) => p.track_stock && p.stock <= p.stock_minimum).length
            : null,
    ...
    const sellPriceLabel =
        storeType === 'session' || storeType === 'parking' ? 'Tarif' :
        storeType === 'rental'      ? 'Tarif Sewa' :
        storeType === 'service'     ? 'Tarif Jasa' :
        storeType === 'hospitality' || storeType === 'ticket' ? 'Tarif' :
        'Harga Jual';
    ...
    return (
        <AuthenticatedLayout
            header={
                <div className="flex w-full items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-slate-800">Manajemen {pageTitle}</h2>
                    <Link
                    ...
                    >
                        ...
                        <span className="hidden sm:inline">Tambah {pageTitle}</span>
        ...
        >
            ...
            <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[
                    { label: `Total ${pageTitle}`, value: stats.total,    icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10',                                                                                                  color: 'text-slate-700' },
                ...
                <div className="border-b border-slate-100 p-4">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                        <p className="text-xs text-slate-500">
                            Menampilkan <span className="font-semibold text-slate-700">{filtered.length}</span> dari {products.length} {pageTitle.toLowerCase()}
                        </p>
                    </div>
                </div>
                ...
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
                        ...
                        <h3 className="mt-4 text-base font-semibold text-slate-800">
                            {hasFilters ? `${pageTitle} tidak ditemukan` : `Belum ada ${pageTitle.toLowerCase()}`}
                        </h3>
                        <p className="mt-1 max-w-sm text-sm text-slate-500">
                            {hasFilters ? 'Coba kata kunci atau filter lain.' : `Mulai dengan menambahkan ${pageTitle.toLowerCase()} pertama.`}
                        </p>
                        {!hasFilters && (
                            <Link
                                href={route('admin.products.create')}
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/60 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                                        ...
                                        {showMargin && <th className="px-5 py-3.5 text-right hidden xl:table-cell">Harga Beli</th>}
                                        <th className="px-5 py-3.5 text-right">{sellPriceLabel}</th>
                                        {showRateHour  && <th className="px-5 py-3.5 text-right hidden xl:table-cell">Per Jam</th>}
                                        {showCapacity  && <th className="px-5 py-3.5 text-center hidden lg:table-cell">Kapasitas</th>}
                                        {showMaxGuests && <th className="px-5 py-3.5 text-center hidden lg:table-cell">Max Tamu</th>}
                                        {showDuration  && <th className="px-5 py-3.5 text-center hidden lg:table-cell">Durasi</th>}
                                        {showDeposit   && <th className="px-5 py-3.5 text-right hidden lg:table-cell">Deposit</th>}
                                        {showPrepTime  && <th className="px-5 py-3.5 text-center hidden lg:table-cell">Prep</th>}
                                        {showStock     && <th className="px-5 py-3.5 text-center">Stok</th>}
                                        ...
                                        <tr key={product.id} className="transition hover:bg-slate-50/70">
                                            ...
                                            {showCapacity && (
                                                <td className="px-5 py-4 text-center text-slate-600 hidden lg:table-cell">
                                                    {product.capacity ?? <span className="text-slate-300">—</span>}
                                                </td>
                                            )}
                                            {/* Max Guests — hospitality */}
                                            {showMaxGuests && (
                                                <td className="px-5 py-4 text-center text-slate-600 hidden lg:table-cell">
                                                    {product.max_guests ? `${product.max_guests} tamu` : <span className="text-slate-300">—</span>}
                                                </td>
                                            )}
                                            ...
                                            {showPrepTime && (
                                                <td className="px-5 py-4 text-center text-slate-500 hidden lg:table-cell">
                                                    {product.preparation_time ? `${product.preparation_time}m` : <span className="text-slate-300">—</span>}
                                                </td>
                                            )}
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    {['retail', 'fnb'].includes(storeType) && (
                                                        <Link href={route('admin.products.variants.index', product.id)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 transition hover:bg-amber-50 hover:text-amber-600" title="Kelola Varian">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className="mt-2 flex flex-wrap items-center gap-3">
                                                ...
                                                {showRateHour && product.price_per_hour > 0 && (
                                                    <span className="text-xs text-slate-500">
                                                        /jam: Rp {Number(product.price_per_hour).toLocaleString('id-ID')}
                                                    </span>
                                                )}
                                                {showDuration && (
                                                    <span className="text-xs text-slate-500">
                                                ...
                                                {showDeposit && product.deposit_amount > 0 && (
                                                    <span className="text-xs text-slate-500">
                                                        Deposit: Rp {Number(product.deposit_amount).toLocaleString('id-ID')}
                                                    </span>
                                                )}
                                                ...
                                                {showStock && <StockBadge product={product} />}
                                            ...
                                            <div className="mt-3 flex items-center gap-1">
                                                {['retail', 'fnb'].includes(storeType) && (
                                                    <Link href={route('admin.products.variants.index', product.id)} className="inline-flex h-8 items-center gap-1 rounded-lg px-2.5 text-xs font-medium text-amber-600 transition hover:bg-amber-50">

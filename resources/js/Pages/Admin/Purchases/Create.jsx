                    ...
                    {filtered.length === 0 ? (
                        ...
                        >
                            <div className="min-w-0 flex-1">
                                <p className={`truncate font-medium ${i === idx ? 'text-indigo-700' : 'text-slate-800'}`}>{p.name}</p>
                            ...
                            <div className="flex shrink-0 items-center gap-3">
                                <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                                    ...
                                    {p.stock ?? 0} {p.base_unit || unitLabel}
                                </span>
                                {p.cost_price > 0 && (
                                    <span className="text-xs text-slate-400">{fmtRp(p.cost_price)}</span>
                                )}
                            </div>
                        </button>
                    ))}
...
export default function Create({ suppliers, products, paymentMethods, storeType }) {
    ...
    /* ── Totals ── */
    const subtotal   = useMemo(() => data.items.reduce((s, i) => s + i.quantity * i.cost_price, 0), [data.items]);
    ...
    const handlePick = (product) => {
        ...
        setPendingPrice(product.cost_price > 0 ? String(product.cost_price) : '');
        setPendingQty(1);
    ...
    const handleAdd = () => {
        ...
        const existIdx = data.items.findIndex((i) => i.product_id === pendingProduct.id);
        if (existIdx >= 0) {
    ...
    const handleAddKey = (e) => { if (e.key === 'Enter') { e.preventDefault(); handleAdd(); } };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-slate-800">Tambah Pembelian</h2>
                </div>
            }
        >
            ...
            <form onSubmit={submit}>
                <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
                    ...
                    <div className="space-y-5 lg:col-span-2">
                        ...
                        >
                            <div className="space-y-4">
                                {/* Step 1: cari produk */}
                                <ProductCombobox products={products} storeType={storeType} onPick={handlePick} />
                                ...
                                {pendingProduct && (
                                    <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4">
                                        <div className="mb-3 flex items-center justify-between">
                                            <div>
                                                <p className="text-sm font-semibold text-indigo-800">{pendingProduct.name}</p>
                                                <p className="text-xs text-indigo-500">{pendingProduct.sku}</p>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-12 items-end gap-3">
                                            <div className="col-span-4 sm:col-span-3">
                                                <label className="mb-1 block text-xs font-semibold text-indigo-700">Qty</label>
                                                <input
                                                    ref={qtyRef}
                                                    type="number"
                                                    value={pendingQty}
                                            ...
                                            <div className="col-span-5 sm:col-span-5">
                                                <label className="mb-1 block text-xs font-semibold text-indigo-700">Harga Beli / Satuan</label>
                                                <div className="relative">
                                                    <span className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-xs text-indigo-400">Rp</span>
                                                    <input
                                                        type="number"
                                                        value={pendingPrice}
                                        ...
                                        {pendingQty > 0 && pendingPrice > 0 && (
                                            <p className="mt-2 text-right text-xs text-indigo-600 font-medium">
                                                Subtotal: {fmtRp(Number(pendingQty) * Number(pendingPrice))}
                                            </p>
                                        )}
                                    </div>
                                )}
                                ...
                                {errors.items && (
                                    <p className="text-xs text-red-600">{typeof errors.items === 'string' ? errors.items : 'Minimal 1 item wajib ditambahkan'}</p>
                                )}

                                {/* Daftar item */}
                                {data.items.length === 0 ? (
                                    <div className="rounded-2xl border-2 border-dashed border-slate-200 py-10 text-center">
                                        <svg className="mx-auto mb-3 h-10 w-10 text-slate-300" fill="none" viewBox="0 0 24 24" strokeWidth={1.3} stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                                        </svg>
                                        <p className="text-sm text-slate-400">Cari dan pilih produk di atas untuk menambahkan item</p>
                                    </div>
                                ) : (
                                            ...
                                            <tbody className="divide-y divide-slate-100">
                                                {data.items.map((item, i) => (
                                                    <tr key={i} className="hover:bg-slate-50/70 transition">
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <div className="relative">
                                                                <span className="pointer-events-none absolute inset-y-0 left-2 flex items-center text-xs text-slate-400">Rp</span>
                                                                <input
                                                                    type="number"
                                                        ...
                                                        <td className="px-4 py-3 text-right text-sm font-semibold text-slate-700">
                                                            {fmtRp(item.quantity * item.cost_price)}
                                                        </td>
                        ...
                        {/* Pembayaran */}
                        <SectionCard title="Pembayaran" subtitle="Isi jika sudah ada pembayaran. Kosongkan jika bayar nanti.">
                            <div className="space-y-4">
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-5">
                        ...
                        {data.items.length === 0 && (
                            <p className="text-center text-xs text-slate-400">Tambahkan minimal 1 item untuk menyimpan</p>
                        )}
                    </div>

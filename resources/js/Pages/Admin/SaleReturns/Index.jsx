...
export default function Index({ saleReturns }) {
    ...
    const stats = useMemo(() => {
        const total = saleReturns.length;
        const completed = saleReturns.filter((r) => r.status === 'completed').length;
        const cancelled = saleReturns.filter((r) => r.status === 'cancelled').length;
        const totalValue = saleReturns.reduce((sum, r) => sum + Number(r.total_amount || 0), 0);
        ...
        <AuthenticatedLayout
            header={
                <div className="flex w-full items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-slate-800">Retur Penjualan</h2>
                    <Link
                ...
                <div className="border-b border-slate-100 p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder="Cari nomor retur, pelanggan, atau penjualan..."
                    ...
                    <div className="pt-4 flex items-center justify-between">
                        <p className="text-xs text-slate-500">
                            ...
                            dari{" "}
                            <span className="font-semibold text-slate-700">{saleReturns.length}</span>{" "}
                            retur
                        </p>
                    </div>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50/60">
                            <tr>
                                ...
                                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Penjualan Asal</th>
                                <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Pelanggan</th>
                            ...
                            {filtered.length === 0 ? (
                                <tr>
                                    <td colSpan={8} className="px-5 py-16 text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="mt-4 text-sm font-medium text-slate-600">Belum ada retur penjualan</p>
                                ...
                                filtered.map((retur) => (
                                    <tr key={retur.id} className="transition hover:bg-slate-50/50">
                                        ...
                                        <td className="whitespace-nowrap px-5 py-4">
                                            <span className="text-sm text-slate-600">{retur.sale?.sale_no || '-'}</span>
                                        </td>
                                        <td className="whitespace-nowrap px-5 py-4">
                                            <span className="text-sm text-slate-600">{retur.customer?.name || '-'}</span>
                                        </td>
                        ...
                        <div key={retur.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                            ...
                            <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                                <div>
                                    ...
                                    <p className="font-medium text-slate-700">{retur.customer?.name || '-'}</p>
                                </div>

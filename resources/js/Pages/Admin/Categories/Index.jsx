        ...
        <AuthenticatedLayout
            header={
                <div className="flex w-full items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-slate-800">Kategori</h2>
                    <Link
                ...
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
                        ...
                        <p className="mt-1 max-w-sm text-sm text-slate-500">
                            {search ? 'Coba kata kunci lain.' : 'Mulai dengan menambahkan kategori pertama untuk produk kamu.'}
                        </p>
                        {!search && (
                            <Link
                                href={route('admin.categories.create')}
                    ...
                    <CategoryList items={filtered} onDelete={setTarget} />
                )}
            </div>
    ...
    return (
        <>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-slate-100 bg-slate-50/60 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                            ...
                            <th className="px-6 py-3.5 text-center">Produk</th>
                        ...
                        {items.map((cat) => (
                            <tr key={cat.id} className="transition hover:bg-slate-50/70">
                                ...
                                <td className="max-w-xs px-6 py-4">
                                    <span className="line-clamp-1 text-slate-500">{cat.description || '—'}</span>
                                </td>
            ...
            <div className="divide-y divide-slate-100 md:hidden">
                {items.map((cat) => (
                    <div key={cat.id} className="flex items-start gap-3 p-4">
                        <CategoryBadge name={cat.name} />

        ...
        <AuthenticatedLayout
            header={
                <div className="flex w-full items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-slate-800">Meja Cafe</h2>
                    <Link
                        href={route('admin.cafe-tables.create')}
                ...
                <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                        ...
                        <select
                            ...
                            className="rounded-xl border border-slate-300 px-3 py-2.5 text-sm shadow-sm transition focus:border-primary-500 focus:ring-2 focus:ring-primary-200"
                        >
                    ...
                    <p className="text-sm text-slate-500">
                        Total <span className="font-semibold text-slate-700">{filtered.length}</span> meja
                    </p>
                </div>
                                        ...
                                        <tr key={t.id} className="transition hover:bg-slate-50/70">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="font-medium text-slate-800">{t.table_number}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-slate-600">{t.branch?.name || '—'}</td>
                                            <td className="px-6 py-4 text-center font-medium text-slate-700">{t.capacity} orang</td>
                                            <td className="px-6 py-4 text-center">
                                                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[t.status] || 'bg-slate-100 text-slate-600'}`}>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                {t.is_active ? (
                                                    ...
                                                    <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-500">Nonaktif</span>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                <p className="truncate font-medium text-slate-800">{t.table_number}</p>
                                                <p className="text-xs text-slate-400">{t.branch?.name || '—'}</p>
                                            </div>
                                        </div>
                                        <span className={`inline-flex shrink-0 items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[t.status] || 'bg-slate-100 text-slate-600'}`}>
                                            {STATUS_LABELS[t.status] || t.status}
                                        </span>
                                    </div>
                                    <div className="mt-3 flex items-center gap-4 text-sm text-slate-500">
                                        <span>Kapasitas: <span className="font-medium text-slate-700">{t.capacity} orang</span></span>
                                        {t.is_active ? (
                                            ...
                                            <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500">Nonaktif</span>
                                        )}
                                    </div>

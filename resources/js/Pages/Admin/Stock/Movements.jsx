...
const MOVEMENT_TYPES = {
    purchase_in:         { label: 'Pembelian Masuk',        color: 'bg-emerald-100 text-emerald-700', icon: 'arrow-down',  desc: 'Stok bertambah karena pembelian dari supplier' },
    purchase_out:        { label: 'Pembelian Dibatalkan',    color: 'bg-red-100 text-red-700',         icon: 'arrow-up',    desc: 'Pembelian dibatalkan, stok dikurangi' },
            ...
            header={
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Catatan setiap perubahan jumlah stok di gudang ini</p>
                    </div>
                </div>
            }
        >
                    ...
                    <div className="relative" ref={dropdownRef}>
                        ...
                        >
                            ...
                            {selectedProduct ? (
                                ...
                                <svg className={`ml-auto h-4 w-4 shrink-0 text-slate-400 transition ${dropdownOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                            )}
                        </button>
                        {dropdownOpen && (
            ...
            {movements.data.length > 0 && (
                <div className="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-3">
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        ...
                        <p className="mt-1 text-2xl font-bold text-slate-800">{movements.total.toLocaleString('id-ID')}</p>
                    </div>
                    <div className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
                        ...
                        <p className="mt-1 text-2xl font-bold text-emerald-700">+{summary.totalIn.toLocaleString('id-ID')}</p>
                    </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-slate-100 bg-slate-50/60">
                                <th className="px-6 py-3.5 font-medium text-slate-500">Waktu</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Produk</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Tipe</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Cabang</th>
                                <th className="px-6 py-3.5 text-right font-medium text-slate-500">Jumlah</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">No. Ref</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Keterangan</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-16 text-center">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p className="text-sm text-slate-400">Tidak ada data pergerakan stok.</p>
                                            {hasActiveFilter && <p className="text-xs text-slate-300">Coba ubah filter yang dipilih.</p>}
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                movements.data.map((m) => {
                                    const typeInfo = MOVEMENT_TYPES[m.movement_type] || { label: m.movement_type, color: 'bg-slate-100 text-slate-600', desc: '' };
                                    const inBound = isIn(m.movement_type);
                                    return (
                                        <tr key={m.id} className="transition hover:bg-slate-50/50">
                                            <td className="px-6 py-3.5">
                                                <p className="text-slate-700">{fmtDate(m.moved_at)}</p>
                                                <p className="text-xs text-slate-400">{fmtTime(m.moved_at)}</p>
                                            </td>
                                            <td className="px-6 py-3.5">
                                            ...
                                            <td className="px-6 py-3.5">
                                                ...
                                                {typeInfo.desc && <p className="mt-0.5 max-w-[200px] text-[10px] leading-tight text-slate-400">{typeInfo.desc}</p>}
                                            </td>
                                            <td className="px-6 py-3.5">
                                                <span className="text-xs text-slate-500">{m.branch?.name ?? '-'}</span>
                                            </td>
                                            ...
                                            <td className="px-6 py-3.5">
                                                <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{m.reference_no ?? '-'}</span>
                                            </td>
                                            <td className="max-w-[220px] px-6 py-3.5 text-xs text-slate-500" title={m.notes ?? ''}>
                ...
                {movements.last_page > 1 && (
                    <div className="flex items-center justify-between border-t border-slate-100 px-6 py-3">
                        <p className="text-xs text-slate-500">Menampilkan {movements.from}–{movements.to} dari {movements.total.toLocaleString('id-ID')} record</p>

    ...
    return (
            ...
            header={
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-lg font-semibold text-slate-800">Transfer Stok</h2>
                    </div>
                    <Link href={route('admin.stock-transfers.create')} className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 transition hover:from-indigo-600 hover:to-violet-700">
        ...
        >
            ...
            <div className="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                <StatCard title="Diterima" value={stats.received} color="emerald" />
            ...
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="relative flex-1">
                    ...
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari no. transfer..." className="w-full rounded-xl border border-slate-300 py-2.5 pl-10 pr-4 text-sm shadow-sm transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" />
                </div>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-slate-100 bg-slate-50/60">
                                <th className="px-6 py-3.5 font-medium text-slate-500">No. Transfer</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Tanggal</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Dari</th>
                                <th className="px-6 py-3.5 font-medium text-slate-500">Ke</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filtered.length === 0 ? (
                                <tr><td colSpan={7} className="px-6 py-12 text-center text-sm text-slate-400">Tidak ada data transfer stok.</td></tr>
                            ) : (
                                filtered.map((t) => (
                                    <tr key={t.id} className="transition hover:bg-slate-50/50">
                                        <td className="px-6 py-3.5">
                                            <Link href={route('admin.stock-transfers.show', t.id)} className="font-semibold text-indigo-600 hover:text-indigo-800">{t.transfer_no}</Link>
                                        </td>
                                        <td className="px-6 py-3.5 text-slate-600">{fmtDate(t.transfer_date)}</td>
                                        <td className="px-6 py-3.5 text-slate-600">{t.from_branch?.name ?? '-'}</td>
                                        <td className="px-6 py-3.5 text-slate-600">{t.to_branch?.name ?? '-'}</td>
                                        <td className="px-6 py-3.5 text-slate-600">{t.user?.name ?? '-'}</td>
                                        <td className="px-6 py-3.5 text-center"><StatusBadge status={t.status} /></td>
                                        <td className="px-6 py-3.5 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <Link href={route('admin.stock-transfers.show', t.id)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-600" title="Lihat Detail">
                                                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                                </Link>
                                                {(t.status === 'pending' || t.status === 'cancelled') && (
                                                    <button onClick={() => setConfirmDelete(t)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-red-50 hover:text-red-500" title="Hapus">
            ...
            {confirmDelete && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onMouseDown={() => !processing && setConfirmDelete(null)}>
                    ...
                    <div className="relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
                        <h3 className="text-lg font-semibold text-slate-900">Hapus Transfer?</h3>
                        <p className="mt-2 text-sm text-slate-500">
                            Transfer <strong>{confirmDelete.transfer_no}</strong> akan dihapus permanen.
                        </p>

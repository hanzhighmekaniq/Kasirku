        ...
        <AuthenticatedLayout
            ...
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="relative flex-1">
                    ...
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari no. waste..." className="w-full rounded-xl border border-border py-2.5 pl-10 pr-4 text-sm shadow-sm transition outline-none focus:border-ring focus:ring-2 focus:ring-ring" />
                </div>
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-xl border border-border bg-card px-3 py-2.5 text-sm text-foreground shadow-sm transition outline-none focus:border-ring focus:ring-2 focus:ring-ring">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-border bg-muted/50">
                                <th className="px-6 py-3.5 font-medium text-muted-foreground">No. Waste</th>
                                <th className="px-6 py-3.5 font-medium text-muted-foreground">Tanggal</th>
                                <th className="px-6 py-3.5 font-medium text-muted-foreground">Oleh</th>
                                <th className="px-6 py-3.5 font-medium text-muted-foreground">Total Item</th>
                                <th className="px-6 py-3.5 font-medium text-muted-foreground">Total Kerugian</th>
                                        ...
                                        <tr key={w.id} className="transition hover:bg-muted/50">
                                            ...
                                            <td className="px-6 py-3.5 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    {w.status === 'draft' && (
                                                        <button onClick={() => setConfirmDelete(w)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition hover:bg-red-50 hover:text-red-500" title="Hapus">
...
function StatCard({ title, value, color }) {
    const colors = { slate: 'border-border bg-card', amber: 'border-amber-200 bg-amber-50', emerald: 'border-emerald-200 bg-emerald-50', red: 'border-red-200 bg-red-50' };
    const textColors = { slate: 'text-foreground', amber: 'text-amber-700', emerald: 'text-emerald-700', red: 'text-red-700' };
    return (

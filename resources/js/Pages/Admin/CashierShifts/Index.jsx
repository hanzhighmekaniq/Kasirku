...
const fmtDt  = (d) => d ? new Date(d).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" }) : "-";
...
export default function Index({ shifts, activeShift, filters, canOpen, canManage }) {
    ...
    const applyFilter = (s) => { setStatus(s); navigate(s, search); };

    const applySearch = (e) => { e.preventDefault(); navigate(status, search); };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between gap-4">
                    <h2 className="text-base font-semibold text-slate-800">Shift Kasir</h2>
                    {canOpen && (
                        <Link
            ...
            <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3">
                        <div>
                            <p className="text-sm font-semibold text-emerald-800">
                                Shift Aktif:{" "}
                                <span className="font-mono">{activeShift.shift_no}</span>
                            </p>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="ml-auto flex items-center gap-1">
                        {[["", "Semua"], ["open", "Berjalan"], ["closed", "Tutup"]].map(([val, lbl]) => (
                            <button
                                key={val}
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[640px] text-left text-sm">
                            <thead className="border-b border-slate-100 bg-slate-50">
                                <tr>
                                    {["No. Shift", "Kasir", "Cabang", "Dibuka", "Ditutup", "Total Penjualan", "Status", ""].map((h, i) => (
                                        <th key={i} className="px-4 py-3 text-xs font-semibold text-slate-500">{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {list.length === 0 ? (
                                    ...
                                    <tr key={shift.id} className="hover:bg-slate-50/60">
                                        ...
                                        <td className="px-4 py-3 text-slate-600">{shift.user?.name ?? "-"}</td>
                                        <td className="px-4 py-3 text-slate-500">{shift.branch?.name ?? "Pusat"}</td>
                                        <td className="px-4 py-3 text-xs text-slate-500">{fmtDt(shift.opened_at)}</td>
                                        <td className="px-4 py-3 text-xs text-slate-500">{fmtDt(shift.closed_at)}</td>
                                        <td className="px-4 py-3 font-medium text-slate-700">
                                            {shift.status === "closed" ? fmt(shift.total_sales) : "-"}
                                        </td>
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_CLS[shift.status] ?? "bg-slate-100 text-slate-600"}`}>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-end gap-1">
                                                <Link
                                                    href={route("admin.cashier-shifts.show", shift.id)}
                ...
                >
                    ...
                    >
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            Shift{" "}
                            <span className="font-mono font-semibold">{deleteTarget.shift_no}</span>{" "}

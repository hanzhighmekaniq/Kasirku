        ...
        <div className="flex-1 overflow-y-auto p-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <div className="col-span-2 rounded-2xl bg-white border border-slate-200 p-4">
                    <div className="flex items-end justify-between mb-3">
                        <div>
                            ...
                            <div className="text-[11.5px] text-slate-500 mt-0.5">Pickup Sat 16 · Return Tue 19 · 4 days</div>
                        </div>
                        <div className="flex gap-1">
                            <button className="h-8 w-8 rounded-lg border border-slate-200 grid place-items-center hover:bg-slate-50">‹</button>
                            <button className="h-8 w-8 rounded-lg border border-slate-200 grid place-items-center hover:bg-slate-50">›</button>
                        </div>
                    </div>
                    <div className="grid grid-cols-7 gap-1.5 mb-1.5">
                        {days.map((d) => <div key={d} className="text-center text-[10.5px] uppercase tracking-wider text-slate-500 font-medium py-1">{d}</div>)}
                    </div>
                    ...
                    <div className="mt-4 flex items-center gap-3 text-[11px]">
                        ...
                        <span className="flex items-center gap-1.5"><span className="h-3 w-3 rounded bg-rose-100 border border-rose-200"></span>Unavailable</span>
                ...
                <div className="space-y-4">
                    <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-700 text-white p-5">
                        ...
                        <div className="mt-4 flex gap-2">
                            <span className="inline-flex items-center rounded-md bg-white/10 border border-white/10 px-2 py-0.5 text-[10.5px] font-medium text-white">Auth ok</span>
                            <span className="inline-flex items-center rounded-md bg-white/10 border border-white/10 px-2 py-0.5 text-[10.5px] font-medium text-white">Refund on return</span>
                        </div>
                    </div>
                    <div className="rounded-2xl bg-white border border-slate-200 p-4">
                        <div className="text-[10.5px] uppercase tracking-widest text-slate-500 font-medium">Return Checklist</div>
                        <div className="mt-2 space-y-1.5 text-[12.5px]">
                            {checklist.map((x, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    <span className={`h-4 w-4 rounded-md border ${i < 3 ? "bg-emerald-500 border-emerald-500 text-white grid place-items-center" : "border-slate-300"}`}>{i < 3 ? "✓" : ""}</span>
                                    {x}
                                </div>
                            ))}
                        </div>
            ...
            <div className="flex items-end justify-between mb-3">
                <div>
                    ...
                    <div className="text-[11.5px] text-slate-500 mt-0.5">Filter by category · Camera</div>
                </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
                {items.map((item, i) => (
                    <div key={i} className="rounded-2xl border border-slate-200 bg-white p-3.5 hover:border-indigo-400 hover:shadow-md transition-all">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-[10.5px] uppercase tracking-wider text-slate-500 font-medium">{item.c}</div>
                        ...
                        <div className="mt-3 flex items-baseline justify-between">
                            <div className="text-[15px] font-semibold tabular-nums">{k.fmt(item.p)}<span className="text-[10.5px] text-slate-500 font-normal">{item.unit}</span></div>
                            <button onClick={() => k.addToCart({ id: `rent-${i}`, name: item.n, sell_price: item.p, track_stock: false })} className="h-8 px-3 rounded-lg bg-gradient-to-r from-indigo-500 to-violet-600 text-white text-[11.5px] font-medium shadow-sm">Add</button>
                        </div>
                    </div>
                ))}
            </div>

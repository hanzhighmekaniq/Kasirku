        ...
        <div className="flex-1 overflow-y-auto p-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <div className="col-span-2 rounded-2xl bg-card border border-border p-4">
                    <div className="flex items-end justify-between mb-3">
                        <div>
                            ...
                            <div className="text-[11.5px] text-muted-foreground mt-0.5">Schedule at a glance</div>
                        </div>
                        <div className="flex gap-1.5">
                            <button className="h-8 px-3 rounded-lg border border-border text-[11.5px] hover:bg-muted">Day</button>
                            <button className="h-8 px-3 rounded-lg bg-primary text-primary-foreground text-[11.5px] shadow-sm">Staff</button>
                            <button className="h-8 px-3 rounded-lg border border-border text-[11.5px] hover:bg-muted">Week</button>
                        </div>
                    </div>
                    <div className="flex border-b border-border mb-1">
                        <div className="w-16"></div>
                        {hours.map((h) => (
                            <div key={h} className="flex-1 border-r last:border-r-0 border-border text-center py-2 text-[10.5px] text-muted-foreground font-medium">{h}</div>
                        ))}
                    </div>
                    {bars.map((b, i) => (
                ...
                <div className="rounded-2xl bg-card border border-border p-4">
                    <div className="flex items-end justify-between mb-3">
                        <div>
                            ...
                            <div className="text-[11.5px] text-muted-foreground mt-0.5">{queue.length} waiting</div>
                        </div>
                    </div>
            ...
            <div className="flex items-end justify-between mb-3">
                <div>
                    ...
                    <div className="text-[11.5px] text-muted-foreground mt-0.5">{staff.length} stylists on shift</div>
                </div>
            </div>
            <div className="grid grid-cols-4 gap-3 mb-6">
                {staff.map((s, i) => (
                    <div key={i} className="rounded-2xl border border-border bg-card p-3.5 transition-all cursor-pointer hover:border-indigo-400">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-[10.5px] text-muted-foreground mt-0.5">{s.role}</div>
                            </div>
                        </div>
                        <div className="mt-3 flex items-center justify-between text-[11px]">
                            <span className={s.busy ? "text-destructive font-medium" : "text-success font-medium"}>{s.avail}</span>
                            {s.rate && <span className="text-muted-foreground">{s.rate}</span>}
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="flex items-end justify-between mb-3">
                <div>
                    ...
                    <div className="text-[11.5px] text-muted-foreground mt-0.5">Select to add to booking</div>
                </div>
            </div>

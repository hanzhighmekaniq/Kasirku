        ...
        <div className="flex-1 overflow-y-auto p-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            <div className="flex items-end justify-between mb-3">
                <div>
                    ...
                    <div className="text-[11.5px] text-muted-foreground mt-0.5">Today · Nov 12</div>
                </div>
                <button className="h-8 px-3 rounded-lg border border-border text-[11.5px] hover:bg-muted">All events →</button>
            </div>
                ...
                <div className="rounded-2xl bg-card border border-border p-4">
                    <div className="flex items-end justify-between mb-3">
                        <div>
                            ...
                            <div className="text-[11.5px] text-muted-foreground mt-0.5">{times.length} showtimes today</div>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        {times.map((t, i) => (
                            <button key={i} onClick={() => setSelectedTime(i)} className={`rounded-xl border px-3 py-2.5 text-left transition-all ${i === selectedTime ? "bg-primary text-white border-primary shadow-sm" : "bg-card border-border hover:border-indigo-400"}`}>
                                <div className="text-[13px] font-semibold">{t}</div>
                                <div className={`text-[10.5px] mt-0.5 ${i === selectedTime ? "text-white/70" : "text-muted-foreground"}`}>Studio 4 · {k.fmt(14000)}</div>
                            </button>
                        ))}
                    </div>
                </div>
                <div className="col-span-2 rounded-2xl bg-card border border-border p-4">
                    <div className="flex items-end justify-between mb-3">
                        <div>
                            ...
                            <div className="text-[11.5px] text-muted-foreground mt-0.5">{selectedSeats.length} seats selected · Row E</div>
                        </div>
                    ...
                    <div className="flex flex-col items-center gap-1.5">
                        {rows.map((r) => (
                            <div key={r} className="flex items-center gap-1.5">
                                <div className="w-4 text-[10.5px] text-muted-foreground/60 font-medium">{r}</div>
            ...
            <div className="flex items-end justify-between mb-3">
                <div>
                    ...
                    <div className="text-[11.5px] text-muted-foreground mt-0.5">Snacks & merch</div>
                </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
                {addons.map((a, i) => (
                    <div key={i} className="rounded-xl border border-border bg-card p-3 flex items-center gap-3">
                        ...
                        <button onClick={() => k.addToCart({ id: `addon-${i}`, name: a.n, sell_price: a.p, track_stock: false })} className="h-7 w-7 rounded-md border border-border grid place-items-center text-muted-foreground hover:bg-muted">+</button>
                    </div>
                ))}
            </div>
        </div>

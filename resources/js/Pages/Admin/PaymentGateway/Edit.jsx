...
const PROVIDER_METHODS = {
    midtrans: ["qris", "gopay", "shopeepay", "dana", "ovo", "bca_va", "mandiri_va", "bri_va", "bni_va", "permata_va"],
    xendit: ["qris", "bca_va", "mandiri_va", "bri_va", "bni_va", "gopay", "ovo", "dana"],
    doku: ["qris", "bca_va", "mandiri_va", "bri_va", "bni_va", "permata_va"],
    duitku: ["qris", "bca_va", "mandiri_va", "bri_va", "bni_va", "gopay", "ovo", "shopeepay"],
};

export default function Edit({ gateway }) {
    const { flash } = usePage().props;
    const providerCfg = PROVIDER_CONFIG[gateway.provider] ?? PROVIDER_CONFIG.midtrans;
    const allowedMethods = PROVIDER_METHODS[gateway.provider] ?? [];

    const { data, setData, put, processing, errors } = useForm({
        is_active: gateway.is_active ?? false,
        environment: gateway.environment ?? "sandbox",
        server_key: gateway.has_server_key ? "••••••••" : "",
        client_key: gateway.has_client_key ? "••••••••" : "",
        merchant_id: gateway.merchant_id ?? "",
        enabled_methods: gateway.enabled_methods ?? [],
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route("admin.payment-gateway.update", { store_payment_gateway: gateway.id }));
    };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-foreground">Edit {providerCfg.label}</h2>
                </div>
            }
        >
            <Head title={`Edit ${providerCfg.label}`} />
            ...
            <div className="mx-auto max-w-2xl">
                <SectionCard title={`Konfigurasi ${providerCfg.label}`} subtitle="Ubah pengaturan payment gateway.">
                        ...
                        {providerCfg.fields.map((f) => (
                            <Field key={f.key} label={f.label} error={errors[f.key]}>
                                <input
                                    ...
                                    placeholder={gateway[`has_${f.key}`] ? "•••••••• (tersimpan)" : f.placeholder}

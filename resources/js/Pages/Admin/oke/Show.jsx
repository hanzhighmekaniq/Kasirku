...
const STATUS_CONFIG = {
    ...
    completed: { label: 'Selesai', className: 'bg-emerald-100 text-emerald-700', dotColor: 'bg-emerald-500' },
...
export default function Show({ purchase }) {
    ...
    const confirmStatusChange = () => {
        setProcessing(true);
        router.patch(route('admin.purchases.updateStatus', purchase.id), { status: nextStatus }, {
            preserveScroll: true,
            onFinish: () => {
                setProcessing(false);
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-slate-800">Detail Pembelian</h2>
                </div>
            }
        >
            ...
            <div className="mx-auto max-w-4xl space-y-6">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="flex flex-col gap-4 border-b border-slate-100 p-6 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">No. Pembelian</p>
                            <h3 className="mt-1 text-xl font-bold text-slate-800">{purchase.purchase_no}</h3>
                        </div>
                        <div className="flex items-center gap-2">
                            <StatusBadge status={purchase.status} large />
                    ...
                    <div className="grid grid-cols-1 gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3">
                        <div>
                            ...
                            <p className="mt-0.5 text-sm font-medium text-slate-800">{purchase.supplier?.name || '—'}</p>
                        </div>
                        <div>
                            <p className="text-xs font-medium text-slate-500">Tanggal</p>
                            <p className="mt-0.5 text-sm font-medium text-slate-800">{formatDate(purchase.purchase_date)}</p>
                        </div>
                        <div>
                            <p className="text-xs font-medium text-slate-500">Dibuat oleh</p>
                            <p className="mt-0.5 text-sm font-medium text-slate-800">{purchase.user?.name || '—'}</p>
                        </div>
                        {purchase.notes && (
                            <div className="sm:col-span-2 lg:col-span-3">
                                <p className="text-xs font-medium text-slate-500">Catatan</p>
                                <p className="mt-0.5 text-sm text-slate-700 whitespace-pre-wrap">{purchase.notes}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 bg-slate-50/60 px-6 py-4">
                        <h3 className="text-sm font-semibold text-slate-900">Item Pembelian ({purchase.items?.length || 0})</h3>
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200 text-sm">
                            <thead className="bg-white">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Harga Beli</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {purchase.items?.map((item, idx) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="px-4 py-3 text-right text-slate-700">{formatRupiah(item.cost_price)}</td>
                                        <td className="px-4 py-3 text-right font-medium text-slate-800">{formatRupiah(item.subtotal)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 bg-slate-50/60 px-6 py-4">
                        <h3 className="text-sm font-semibold text-slate-900">Ringkasan Keuangan</h3>
                    </div>
                    <div className="p-6">
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                <span className="text-slate-500">Subtotal</span>
                                <span className="font-medium text-slate-700">{formatRupiah(purchase.subtotal)}</span>
                            </div>
                            {purchase.discount_amount > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-slate-500">Diskon</span>
                                    <span className="font-medium text-rose-600">-{formatRupiah(purchase.discount_amount)}</span>
                                </div>
                            )}
                            {purchase.tax_amount > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-slate-500">Pajak</span>
                                    <span className="font-medium text-slate-700">{formatRupiah(purchase.tax_amount)}</span>
                                </div>
                            )}
                            {purchase.shipping_amount > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-slate-500">Ongkos Kirim</span>
                                    <span className="font-medium text-slate-700">{formatRupiah(purchase.shipping_amount)}</span>
                                </div>
                            )}
                            <div className="flex justify-between border-t border-slate-200 pt-2">
                                <span className="font-semibold text-slate-800">Grand Total</span>
                                <span className="text-lg font-bold text-indigo-600">{formatRupiah(purchase.grand_total)}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500">Dibayar</span>
                                <span className="font-medium text-emerald-600">{formatRupiah(purchase.paid_amount)}</span>
                            </div>
                            {remaining > 0 && (
                                <div className="flex justify-between">
                    ...
                    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-slate-200 text-sm">
                                <thead className="bg-white">
                                    <tr>
                                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Tanggal Bayar</th>
                                        <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Metode</th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {purchase.payments.map((payment) => (
                                        <tr key={payment.id}>
                                            <td className="px-4 py-3 text-slate-700">{formatDateTime(payment.paid_at)}</td>
                                            <td className="px-4 py-3 text-slate-700">{payment.payment_method?.name || '—'}</td>
                                            <td className="px-4 py-3 text-right font-medium text-slate-800">{formatRupiah(payment.amount)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                {isCancelled && (
                    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div className="p-6">
                            <p className="text-sm text-slate-500 italic">Pembelian ini telah dibatalkan.</p>
                        </div>
                    </div>
                )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl sm:p-7">
                        <div className="flex items-start gap-4">
                            <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${nextStatus === 'completed' ? 'bg-emerald-100' : 'bg-amber-100'}`}>
                            ...
                            <div className="min-w-0 flex-1">
                                <h3 className="text-base font-semibold text-slate-900">
                                    {nextStatus === 'completed' ? 'Tandai selesai?' : 'Batalkan pembelian?'}
                                </h3>
                                <p className="mt-1 text-sm text-slate-500">
                                    {nextStatus === 'completed'
                                        ? 'Pembelian akan ditandai sebagai selesai dan stok akan ditambahkan ke gudang.'
                                        : 'Pembelian akan dibatalkan. Stok yang sudah masuk akan dikembalikan.'}
                                </p>
                            </div>
                        </div>
                        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                            ...
                            >
                                {processing ? (
                                    ...
                                    nextStatus === 'completed' ? 'Ya, Selesai' : 'Ya, Batalkan'
                                )}
                            </button>
                        </div>
                    </div>

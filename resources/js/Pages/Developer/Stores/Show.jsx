...
const PLAN_STYLE = {
    ...
    unlimited: {
        label: "Unlimited",
        cls: "bg-primary-50 text-primary-700 ring-primary-100",
    },
};
                                ...
                                {data.employees.length > 0 ? (
                                    <div className="space-y-2">
                                        {data.employees.map((emp) => (
                                            <div
                                                key={emp.id}
                                                className="flex items-start gap-3 rounded-xl border border-slate-100 bg-white p-3.5 hover:border-primary-100 hover:bg-primary-50/30 transition-colors"
                                            >
                                                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary-400 to-primary-600 text-xs font-bold text-white shadow-sm">
                                                    {emp.name
                                                        .charAt(0)
                                                        .toUpperCase()}
                                                </div>
...
export default function Show({ store, owners, allUsers, planFeatures = [] }) {
    ...
    const colorMap = {
        indigo: { bg: "bg-primary-50", text: "text-primary-600" },
                ...
                {store.branches?.length > 0 ? (
                    <div className="divide-y divide-slate-100">
                        {store.branches.map((b, i) => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-xs font-bold text-slate-600 group-hover:bg-primary-50 group-hover:text-primary-600 transition-colors">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-primary-50 hover:text-primary-600 transition-colors"
                                        title="Lihat detail"
                                    >
                                        <svg
                                            className="h-4 w-4"
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="flex flex-wrap gap-1">
                                        {u.roles?.map((r, i) => (
                                            <span
                                                key={i}
                                                className={`rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize ${ROLE_STYLE[r] ?? "bg-primary-50 text-primary-700"}`}
                                            >
                                                {r}
                                            </span>
                                        ))}

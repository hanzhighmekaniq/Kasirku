...
class StockTransferController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            "to_branch_id" =>
                "required|exists:branches,id|different:from_branch_id",
    ...
    public function destroy(StockTransfer $stockTransfer)
    {
        if (!in_array($stockTransfer->status, ["pending", "cancelled"])) {
            return back()->withErrors([
                "status" =>
                    "Hanya transfer pending/dibatalkan yang dapat dihapus.",
            ]);
        }
        ...
        try {
            ...
            if ($request->status === "received") {
                foreach ($stockTransfer->items as $item) {
                    // Stock stays at store level (no branch stock changes)
                    // Only record audit trail via StockMovement

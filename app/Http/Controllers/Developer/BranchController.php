...
class BranchController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // jelas kalau toko sudah melebihi limit plan-nya, supaya sadar dan
        // bisa memutuskan (upgrade plan toko atau override sengaja).
        $store = Store::with('planModel')->find($validated['store_id']);
        if ($store && ! $store->canAddBranch()) {
            return redirect()
                ->route('developer.branches.index')
                ->with(
                    'warning',
                    "Cabang ditambahkan, tapi toko \"{$store->name}\" sekarang melebihi limit plan {$store->planModel?->label} ({$store->branches()->count()}/{$store->effectiveMaxBranches()} cabang).",
                );
        }

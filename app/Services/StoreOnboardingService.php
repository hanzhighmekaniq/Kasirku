...
class StoreOnboardingService
{
    ...
    /**
     * Buat toko dari halaman onboarding (user sudah login, sudah punya akun).
     *
     * Dipakai setelah registrasi — user memilih plan, jenis usaha, dan nama
     * toko di halaman onboarding, lalu store dibuat di sini.
     *
     * Method ini HANYA membuat Store + Branch + Roles + dll, TIDAK membuat
     * User baru. Plan user juga di-update sesuai pilihan.
     */
    public function createStoreFromOnboarding(
        User $user,
        int $storeTypeId,
        int $planId,
        ?string $businessTemplateCode,
        ...
        // Update plan user sesuai pilihan onboarding
        $user->update([
            'plan_id' => $plan->id,
